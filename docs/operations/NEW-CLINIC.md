...
| Что создаётся | Clinic + первый ADMIN | Clinic + ADMIN + подписка TRIAL (basic, 14 дней) + плейбук (услуги + шаблоны уведомлений) |
...
| **Service (услуги)** | плейбук (путь B) или `/crm/settings/services` |
...
| **NotificationTemplate** | плейбук (4 шт.) или сид всех дефолтов (8 шт. + новые ключи): см. ниже |
