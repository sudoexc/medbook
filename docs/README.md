...
| [operations/RUNBOOK.md](operations/RUNBOOK.md) | Контейнеры и соседи по VPS, диагностика, **мониторинг и алерты**, типовые инциденты (502, реалтайм, уведомления, диск, воркер), бэкапы, обслуживание демо-данных |
...
> ✅ Ночной бэкап (база + файлы клиники) включён, восстановление проверено —
> см. «Бэкап и восстановление» в [RUNBOOK.md](operations/RUNBOOK.md).
> ⚠️ Копия хранится **на том же сервере**; забирать её наружу нужно отдельно.
