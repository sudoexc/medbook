      ...
      <DialogContent className="max-h-[90vh] max-w-4xl gap-0 overflow-hidden p-0">
            ...
            <div className="border-b px-4 py-2">
              <div className="relative">
                ...
                <input
                  autoFocus
                  type="text"
                  placeholder={t("labOrder.searchPlaceholder")}
                  value={query}
                                ...
                                <span className="flex-1">
                                  <span className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-[10px] text-muted-foreground">
                                      {t("labOrder.testCount", { count: p.testCodes.length })}
                                    </span>
                                  </span>
                                  {p.description && (
                  ...
                  <div>
                    ...
                    {tests.length === 0 ? (
                      <p className="text-xs text-muted-foreground">
                        {t("labOrder.testsEmpty")}
                      </p>
                    ) : (
                      <ul className="space-y-0.5">
                        {tests.map((test) => {
                          const picked = pickedTests.has(test.code);
                          const isPinned = testFavs.pinned.has(test.code);
                          const recommended =
                            diagnosisCode &&
                            test.commonForCodes.some((c) =>
                              diagnosisCode.toUpperCase().startsWith(c.toUpperCase()),
                            );
                          return (
                            <li key={test.id} className="group relative">
                              <button
                                type="button"
                              ...
                              >
                                ...
                                <span className="flex flex-1 items-center gap-1 truncate text-foreground">
                                  ...
                                  <span className="truncate">{test.nameRu}</span>
                                </span>
                                {test.unit && (
                                  <span className="text-[10px] text-muted-foreground">
                                    {test.unit}
                                ...
                                {recommended && (
                                  <span className="inline-flex items-center gap-0.5 rounded-md bg-amber-100 px-1 text-[10px] font-medium text-amber-700">
                                    {t("labOrder.recommended")}
                                  </span>
                                )}
                                <span className="inline-flex items-center gap-0.5 text-[10px] text-muted-foreground">
                                  <ClockIcon className="size-2.5" />
                                  {t("labOrder.turnaroundHours", { hours: test.turnaroundHours })}
                                </span>
                              </button>
                              <button
                                type="button"
          ...
          <div className="flex flex-col overflow-hidden border-l">
            <div className="border-b px-4 py-2 text-xs">
              ...
              <div className="grid grid-cols-3 gap-1">
                {(
                  [
                    { id: "ROUTINE", label: t("labOrder.urgencyRoutine"), Icon: ClockIcon },
                    { id: "URGENT", label: t("labOrder.urgencyUrgent"), Icon: ZapIcon },
                    { id: "STAT", label: t("labOrder.urgencyStat"), Icon: AlertTriangleIcon },
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 text-xs">
              ...
              {basketTotal === 0 ? (
                ...
                <ul className="space-y-1">
                  ...
                  {[...pickedTests].map((code) => {
                    const test = allTestsByCode.get(code);
                    return (
                      <li
                        key={`t-${code}`}
              ...
              <div className="mt-3">
                <label className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-foreground">
                  {t("labOrder.notesLabel")}
                </label>
                <textarea
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  placeholder={t("labOrder.notesPlaceholder")}
                  rows={2}
        ...
        <DialogFooter className="flex-row items-center justify-between gap-2 px-5 py-3">
          <p className="text-[11px] text-muted-foreground">
            {patientId
              ? t("labOrder.footerHint")
              : t("labOrder.noPatientHint")}
          </p>
